...
async function S(e=1,t=8){const o=`https://paw-hut.b.goit.study/api/animals?page=${e}&limit=${t}`;return(await L.get(o)).data}const ee="https://paw-hut.b.goit.study/api/categories",te={method:"GET",headers:{Accept:"application/json","Content-Type":"application/json","X-Custom-Header":"custom value",mode:"no-cors"}};async function ne(){try{const e=await L(ee,te);if(!e.data)throw new Error(`HTTP error! Status: ${e.status}`);return w.show("Success send"),e.data}catch(e){throw w.show("Error send"),console.error(e),e}}function se(e,t,n=1){if(t<=1)return[1];const s=[1],o=Math.max(2,e-n),a=Math.min(t-1,e+n);o>2&&s.push("...");for(let c=o;c<=a;c++)s.push(c);return a<t-1&&s.push("..."),s.push(t),s}function T({container:e,current:t,total:n}){if(n<=1){e.innerHTML="";return}if(n===2){e.innerHTML=`
  ...
  `}async function pe(){if(!I)return;const e=await de();if(!e.length)return;I.innerHTML=e.map(ue).join(""),document.querySelectorAll(".rating-container").forEach(n=>{const s=Number(n.dataset.score)||0;Q(n,s,5)}),new A(".reviews-swiper",{modules:[N,X],speed:500,spaceBetween:16,slidesPerView:1,grabCursor:!0,pagination:{el:".reviews-slider .swiper-pagination",clickable:!0,dynamicBullets:!0,dynamicMainBullets:1},navigation:{nextEl:".reviews-slider .swiper-button-next",prevEl:".reviews-slider .swiper-button-prev"},breakpoints:{320:{slidesPerView:1,spaceBetween:16},768:{slidesPerView:2,spaceBetween:16},1440:{slidesPerView:2,spaceBetween:28}}})}document.addEventListener("DOMContentLoaded",pe);
